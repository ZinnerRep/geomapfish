...
const projectRule = {
  test: /mypackage_geoportal\/static-ngeo\/js\/.*\.js$/,
  use: {
    loader: 'babel-loader',
    options: {
...
module.exports = {
  output: {
    path: path.resolve(__dirname, 'mypackage_geoportal/static-ngeo/build/'),
  ...
  resolve: {
    alias: {
      mypackage: path.resolve(__dirname, 'mypackage_geoportal/static-ngeo/js'),
    }
  },
};
